...
The bot watches one private Telegram channel. It indexes each new photo's faces, lets a channel member (or an explicitly allowed user ID) send a single-person selfie, and returns likely matching channel photos in albums of up to 10. If the selfie misses, `/faces` shows pages of detected face groups; tapping a face searches for similar photos. `/allfaces` includes every detected face when grouping is wrong. `/more` pages through the current search; `/update` sends matching photos indexed since that search, excluding photos already sent. `/reset-history` clears the user's sent-photo history, allowing `/update` to resend matches. Sending a new image also clears that history, even if the new image cannot be processed. The bot remembers the latest valid selfie or selected face for `/update`; `/status` shows indexing progress.

The bot stores face descriptors, small face thumbnails, Telegram file IDs, post times, each user's latest face query, and IDs of successfully sent results in SQLite. The sent history survives restarts but cannot reconstruct photos delivered before this feature was installed. New channel photos and selfies are processed in memory, but a Telegram Desktop backfill retains private image copies under `data/album-media/` so older results can be sent in albums. Keep `data/` private and back it up. The bot does not verify that a selfie or selected face belongs to the person asking; any channel member or explicitly allowed user can search the event collection. Tell attendees about this before rollout, and keep the channel private.
